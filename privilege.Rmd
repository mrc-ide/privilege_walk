...
pie(table(data_sorted$gender_ethnicity_no_na)[c(1, 2, 4, 3)],
    col = c(col_f_dark, col_f, col_m, col_m_dark))
```
...
for(i in seq_along(job_levs)){
  ...
  points(i, p[1], pch = 19, col = col_f, cex = 1.5)
  segments(i, p[2], i, p[3], col = col_f, lwd = 3)
}
...
for(i in c(gender_ethnicity, "ZOther/NA Non white", "ZOther/NA White")) {
  if(i == gender_ethnicity[1]){
    abline(v = range(which(data_sorted_white_non_white_known_gender$gender_ethnicity %in% i)), col = "grey")
  } else{
    abline(v = max(range(which(data_sorted_white_non_white_known_gender$gender_ethnicity %in% i))), col = "grey")
  }
}

par(xpd = TRUE)
for(i in gender_ethnicity) {
  x <- mean(range(which(data_sorted_white_non_white_known_gender$gender_ethnicity %in% i)))
  y <- 1.1 * n_q
  text(x, y, i, cex = 1.2)
}

i <- "ZOther/NA Non white"
x <- mean(range(which(data_sorted_white_non_white_known_gender$gender_ethnicity %in% i)))
y <- 1.2 * n_q
text(x, y, "Other NW", cex = 1.2)
i <- "ZOther/NA White"
x <- mean(range(which(data_sorted_white_non_white_known_gender$gender_ethnicity %in% i)))
y <- 1.1 * n_q
...
## TODO: 
## order within each group by score? 
